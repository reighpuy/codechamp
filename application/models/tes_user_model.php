...
class tes_user_model extends CI_Model{
	public $table = 'tes_user';
    ...
    function update_exp($tesuser_id){
        $sql = 'UPDATE cc_user SET exp_user=30 WHERE user_id="'.$tesuser_id.'"';
        $this->db->query($sql);
    }

    function update_menit($tesuser_id, $waktu){
        $sql = 'UPDATE tes_user SET tesuser_creation_time=TIMESTAMPADD(MINUTE, '.$waktu.', tesuser_creation_time) WHERE tesuser_id="'.$tesuser_id.'"';
        $this->db->query($sql);
    }
    ...
    function count_by_status_waktu($tesuser_id){
        ...
        $this->db->select('COUNT(tesuser_id) AS hasil')
                 ...
                 ->from($this->table)
                 ->join('tes', 'tes_user.tesuser_tes_id = tes.tes_id');
        return $this->db->get();
    }
    ...
    function count_by_status_waktuuser($tesuser_id, $waktuuser){
        $this->db->select('COUNT(tesuser_id) AS hasil')
                 ->where('(tesuser_id="'.$tesuser_id.'" AND tesuser_status="1")')
                 ->from($this->table)
                 ->join('tes', 'tes_user.tesuser_tes_id = tes.tes_id');
        return $this->db->get();
    }
    ...
    function get_by_tes_group_urut_tanggal($tes_id, $grup_id, $urutkan, $tanggal, $keterangan){
        ...
        $this->db->select('tes_user.*, tes.*, cc_user.*, user_grup.grup_nama, SUM(`tes_soal`.`tessoal_nilai`) AS nilai ')
                 ->where('( '.$sql.' )')
                 ->from($this->table)
                 ->join('cc_user', 'tes_user.tesuser_user_id = cc_user.user_id', 'right')
                 ->join('user_grup', 'cc_user.user_grup_id = user_grup.grup_id')
                 ->join('tes', 'tes_user.tesuser_tes_id = tes.tes_id', 'left')
                 ->join('tes_soal', 'tes_soal.tessoal_tesuser_id = tes_user.tesuser_id', 'left')
                 ->group_by('tes_user.tesuser_id')
                 ->order_by($order);
        return $this->db->get();
    }
	...
	function get_by_tes_group($tes_id, $grup_id){
        ...
        $this->db->select('tes_user.*, tes.tes_nama, cc_user.*, user_grup.grup_nama')
                 ->where('( '.$sql.' )')
                 ->from($this->table)
                 ->join('cc_user', 'tes_user.tesuser_user_id = cc_user.user_id')
                 ->join('user_grup', 'cc_user.user_grup_id = user_grup.grup_id')
                 ->join('tes', 'tes_user.tesuser_tes_id = tes.tes_id')
                 ->order_by('cc_user.nama_user', 'ASC');
        return $this->db->get();
    }

    function get_nilai_by_tes_user($tes_id, $user_id){
        $this->db->select('SUM(`tes_soal`.`tessoal_nilai`) AS nilai')
                 ->where('(tesuser_tes_id="'.$tes_id.'" AND tesuser_user_id="'.$user_id.'")')
                 ->from($this->table)
                 ->join('tes_soal', 'tes_soal.tessoal_tesuser_id = tes_user.tesuser_id');
        return $this->db->get();
    }
	...
	*/
	function get_datatable($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal, $keterangan, $search){
        $sql = 'tesuser_creation_time>="'.$tanggal[0].'" AND tesuser_creation_time<="'.$tanggal[1].'" AND nama_user LIKE "%'.$search.'%"';
		...
		$this->db->select('tes_user.*,user_grup.grup_nama, tes.*, cc_user.*, SUM(`tes_soal`.`tessoal_nilai`) AS nilai ')
                 ->where('( '.$sql.' )')
                 ->from($this->table)
                 ->join('cc_user', 'tes_user.tesuser_user_id = cc_user.user_id')
                 ->join('user_grup', 'cc_user.user_grup_id = user_grup.grup_id')
    ...
    function get_datatable_evaluasi($start, $rows, $tes_id, $urutkan){
        ...
        $this->db->select('tes_soal.tessoal_id, tes_soal.tessoal_jawaban_text, tes.*, soal.*')
                 ->where('(soal_tipe="2" AND tessoal_jawaban_text IS NOT NULL AND tessoal_comment IS NULL '.$sql.' )')
                 ->from($this->table)
                 ->join('tes', 'tes_user.tesuser_tes_id = tes.tes_id')
                 ->join('tes_soal', 'tes_soal.tessoal_tesuser_id = tes_user.tesuser_id')
                 ->join('soal', 'tes_soal.tessoal_soal_id = soal.soal_id')
                 ->order_by($order)
                 ->limit($rows, $start);
        return $this->db->get();
    }
    ...
    function get_datatable_operator($start, $rows, $token){
        $this->db->select('tes_user.*,user_grup.grup_nama, tes.*, cc_user.*, SUM(`tes_soal`.`tessoal_nilai`) AS nilai ')
                 ...
                 ->from($this->table)
                 ->join('user', 'tes_user.tesuser_user_id = cc_user.user_id')
                 ->join('user_grup', 'cc_user.user_grup_id = user_grup.grup_id')
                 ->join('tes', 'tes_user.tesuser_tes_id = tes.tes_id')
                 ->join('tes_soal', 'tes_soal.tessoal_tesuser_id = tes_user.tesuser_id')
                 ->group_by('tes_user.tesuser_id')
