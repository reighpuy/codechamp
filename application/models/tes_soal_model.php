...
class tes_soal_model extends CI_Model{
    ...
    function get_by_tessoal_limit($tessoal_id, $limit){
        $this->db->select('tessoal_id,tessoal_tesuser_id,tessoal_user_ip,tessoal_soal_id,tessoal_jawaban_text,tessoal_nilai,tessoal_ragu,tessoal_creation_time,tessoal_change_time,tessoal_order,tessoal_num_answers,tessoal_comment,soal_id,soal_topik_id,soal_detail,soal_tipe,soal_kunci,soal_difficulty,soal_aktif,soal_inline_answers,soal_auto_next')
    ...
    function get_nilai($tessoal_id){
        $sql = 'SELECT SUM(tessoal_nilai) AS hasil, COUNT(CASE  WHEN tessoal_nilai=0 THEN 1 END) AS jawaban_salah, COUNT(*) AS total_soal FROM tes_soal WHERE tessoal_tesuser_id="'.$tessoal_id.'"';
