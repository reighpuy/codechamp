...
class Tes_dashboard extends Tes_Controller {
    ...
    function get_datatable(){
		...
		$query = $this->tesgrup_model->get_datatable($start, $rows, $grup_id);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->tesgrup_model->get_datatable_count($grup_id)->row()->hasil;
	    ...
	    foreach ($query as $temp) {
			...
			if($this->tes_topik_set_model->count_by_kolom('tset_tes_id', $temp->tes_id)->row()->hasil>0){
	            ...
	            if($this->tes_user_model->count_by_user_tes($user_id, $temp->tes_id)->row()->hasil>0){
	            	...
	            	$record[] = '';
	            	$record[] = '<a href="'.site_url().'/'.$this->url.'/konfirmasi_test/'.$temp->tes_id.'" style="cursor: pointer;" class="btn btn-primary btn-xs">Kerjakan</a>';
	            }
