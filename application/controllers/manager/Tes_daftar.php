...
class Tes_daftar extends Member_Controller {
    ...
    function hapus_tes(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $this->tes_model->delete('tes_id', $tes_id);
            ...
            if($centang=='1'){
            	foreach( $tes_id as $kunci => $isi ) {
					if($isi=="on"){
		            	...
		            	$this->tes_model->delete('tes_id', $kunci);
    ...
    function get_datatable(){
		...
		$query = $this->tes_model->get_datatable($start, $rows, 'tes_nama', $search);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->tes_model->get_datatable_count('tes_nama', $search)->row()->hasil;
	    ...
	    foreach ($query as $temp) {			
			...
			$query_topik = $this->tes_topik_set_model->get_by_kolom('tset_tes_id', $temp->tes_id);
			if($query_topik->num_rows()>0){
				$query_topik = $query_topik->result();
				$data_soal = '';
				foreach($query_topik as $topik){
					$query_topik = $this->topik_model->get_by_kolom_limit('topik_id', $topik->tset_topik_id, 1)->row();
			...
			$query_grup = $this->tesgrup_model->get_by_tes_id($temp->tes_id);
