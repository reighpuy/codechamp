...
class Peserta_kartu extends Member_Controller {
	...
	*/
    public function cetak_kartu($grup_id=null){
		...
		if(!empty($grup_id)){
			$query_user = $this->user_model->get_by_kolom('user_grup_id', $grup_id);
			if($query_user->num_rows()>0){
				$kartu = '';
				$query_user = $query_user->result();
				
				$query_konfig = $this->konfigurasi_model->get_by_kolom_limit('konfigurasi_kode', 'nama', 1);
				$nama = 'Codechamp';
				if($query_konfig->num_rows()>0){
					$nama = $query_konfig->row()->konfigurasi_isi;
				}
				
				$query_group = $this->user_grup_model->get_by_kolom_limit('grup_id', $grup_id, 1);
				$group = 'NULL';
				if($query_group->num_rows()>0){
						...
						<div class="kartu">
							...
							<table>
								<tr>
									...
									<td width="5px">:</td>
									<td width="210px">'.$temp->nama_user.'</td>
								</tr>
								<tr>
									<td>Email</td>
									<td>:</td>
									<td>'.$temp->email_user.'</td>
								</tr>
								<tr>
									<td>Tgl daftar</td>
									<td>:</td>
									<td>'.$temp->user_regdate.'</td>
								</tr>
								<tr>
									<td>Tipe</td>
									<td>:</td>
