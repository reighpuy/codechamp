...
class Peserta_daftar extends Member_Controller
{
	...
	function tambah()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			$data['password_user'] = $this->input->post('tambah-password', true);
			...
			if ($this->user_grup_model->count_by_kolom('grup_id', $data['user_grup_id'])->row()->hasil > 0) {
				if ($this->user_model->count_by_kolom('email_user', $data['email_user'])->row()->hasil > 0) {
					$status['status'] = 0;
	...
	function get_by_id($id = null)
	{
		$data['data'] = 0;
		if (!empty($id)) {
			$query = $this->user_model->get_by_kolom('user_id', $id);
			if ($query->num_rows() > 0) {
				$query = $query->row();
				$data['data'] = 1;
				$data['id'] = $query->user_id;
				$data['password_user'] = $query->password_user;
	...
	function hapus_daftar_siswa()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			foreach ($user_id as $kunci => $isi) {
				if ($isi == "on") {
					$this->user_model->delete('user_id', $kunci);
				}
			}
			$status['status'] = 1;
	...
	function edit()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($pilihan == 'hapus') { //hapus
				$this->user_model->delete('user_id', $id);
				$status['status'] = 1;
				$status['pesan'] = 'Data Peserta berhasil dihapus !';
			} else if ($pilihan == 'simpan') { //simpan
				$data['password_user'] = $this->input->post('edit-password', true);
				$data['nama_user'] = $this->input->post('edit-nama', true);
				$data['email_user'] = $this->input->post('edit-email', true);
				$data['user_grup_id'] = $this->input->post('edit-group', true);

				$this->user_model->update('user_id', $id, $data);
	...
	function get_datatable()
	{
		...
		$query = $this->user_model->get_datatable($start, $rows, 'nama_user', $search, $group);
		$iFilteredTotal = $query->num_rows();

		$iTotal = $this->user_model->get_datatable_count('nama_user', $search, $group)->row()->hasil;
		...
		foreach ($query as $temp) {
			...
			$record[] = ++$i;
			$record[] = $temp->email_user;
			$record[] = stripslashes($temp->nama_user);

			$query_group = $this->user_grup_model->get_by_kolom_limit('grup_id', $temp->user_grup_id, 1)->row();
