...
class Tes_tambah extends Member_Controller {
    ...
    public function index($tes_id=null){
        ...
        $is_edit = 0;
        if(!empty($tes_id)){
        	$query_tes = $this->tes_model->count_by_kolom('tes_id', $tes_id)->row()->hasil;
        	if($query_tes>0){
        		$is_edit = 1;
        	}
        }
        ...
        if($query_group->num_rows()>0){
        	...
        	foreach ($query_group as $temp) {
        		if($is_edit!=0){
        			if($this->tesgrup_model->count_by_tes_and_group($tes_id, $temp->grup_id)->row()->hasil>0){
    ...
    function get_topik_by_modul($modul=null){
        ...
        if(!empty($modul)){
            $data['data']=1;
            $query_topik = $this->topik_model->get_by_kolom('topik_modul_id', $modul);
            if($query_topik->num_rows()){
                $query_topik = $query_topik->result();
                $select = '';
                foreach ($query_topik as $topik) {
                    $jml_soal = $this->soal_model->count_by_kolom('soal_topik_id', $topik->topik_id)->row()->hasil;
    ...
    function tambah_tes(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($cek_nama==1 and $this->tes_model->count_by_kolom('tes_nama', $data['tes_nama'])->row()->hasil>0){
            	$status['status'] = 0;
                ...
                if($is_process==1){
                    ...
                    $this->tesgrup_model->delete('tstgrp_tes_id', $tes_id);
                    foreach ($groups as $group) {
                        ...
                        if($group!=0){
                            $this->tesgrup_model->save($data_group);
                        }
                    }

                    // Mengupdate score maximal
                    $data_tes['tes_max_score'] = $this->hitung_skor($tes_id);
                    $this->tes_model->update('tes_id', $tes_id, $data_tes);
    ...
    function tambah_soal(){
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	// Cek dulu apakah kondisi tes sudah berjalan
            if($this->tes_user_model->count_by_kolom('tesuser_tes_id', $data['tset_tes_id'])->row()->hasil>0){
                $status['status'] = 0;
                $status['pesan'] = 'Data Soal tidak bisa di tambah. Tes masih digunakan.';
            }else{
                $jml_soal = $this->soal_model->count_by_kolom('soal_topik_id', $data['tset_topik_id'])->row()->hasil;
                ...
                if($data['tset_jumlah']<=$jml_soal){
                    ...
                    if($this->tes_topik_set_model->count_by_test_topik($data['tset_tes_id'], $data['tset_topik_id'])->row()->hasil>0){
                        $status['status'] = 0;
                        $status['pesan'] = 'Data soal sudah terdapat pada Tes, silahkan cek kembali';
                    }else{
                        $this->tes_topik_set_model->save($data);
                        ...
                        $this->tes_model->update('tes_id', $data['tset_tes_id'], $data_tes);
                        $status['max_score']=$data_tes['tes_max_score'];
                    }
                }else{
                    $status['status'] = 0;
    ...
    function hapus_soal_by_id($id=null){
		...
		if(!empty($id)){
			// Cek dulu apakah kondisi tes sudah berjalan
            $query_soal = $this->tes_topik_set_model->get_by_kolom_limit('tset_id', $id, 1)->row();
            if($this->tes_user_model->count_by_kolom('tesuser_tes_id', $query_soal->tset_tes_id)->row()->hasil>0){
                $data['data'] = 0;
                $data['pesan'] = 'Data Soal tidak bisa di hapus. Tes masih digunakan.';
            }else{
                // hapus soal dari tes
                $this->tes_topik_set_model->delete('tset_id', $id);
                $data['data'] = 1;
                $data['pesan'] = 'Data Soal berhasil dihapus';

                // update max score di table ctb_test
                $data_tes['tes_max_score'] = $this->hitung_skor($query_soal->tset_tes_id);
                $this->tes_model->update('tes_id', $query_soal->tset_tes_id, $data_tes);
    ...
    private function hitung_skor($tes_id=null){
        ...
        if(!empty($tes_id)){
            ...
            if($query_tes->num_rows()>0){
                ...
                $query_soal = $this->tes_topik_set_model->get_by_kolom('tset_tes_id', $tes_id)->result();
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->tes_model->get_by_kolom('tes_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
    ...
    function get_datatable_soal(){
		...
		$query = $this->tes_topik_set_model->get_datatable($start, $rows, $tes_id);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->tes_topik_set_model->get_datatable_count($tes_id)->row()->hasil;
	    ...
	    foreach ($query as $temp) {
			...
			$query_topik = $this->topik_model->get_by_kolom_limit('topik_id', $temp->tset_topik_id, 1)->row();
			$query_modul = $this->modul_model->get_by_kolom_limit('modul_id', $query_topik->topik_modul_id, 1)->row();
