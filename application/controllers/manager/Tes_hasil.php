...
class Tes_hasil extends Member_Controller {
    ...
    public function index($page=null, $id=null){
        ...
        $query_tes = $this->tes_user_model->get_by_group();
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($pilihan=='hapus'){
                foreach( $tesuser_id as $kunci => $isi ) {
                    if($isi=="on"){
                        $this->tes_user_model->delete('tesuser_id', $kunci);
                    }
                }
            	$status['status'] = 1;
            	...
            	foreach( $tesuser_id as $kunci => $isi ) {
                    if($isi=="on"){
            			...
            			$this->tes_user_model->update('tesuser_id', $kunci, $data_tes);
                    }
                }
            	$status['status'] = 1;
            	...
            	foreach( $tesuser_id as $kunci => $isi ) {
                    if($isi=="on"){
            			...
            			$this->tes_user_model->update_menit($kunci, $waktu);
                    }
                }
            	$status['status'] = 1;
    ...
    function export($tes_id=null, $grup_id=null, $waktu=null, $urutkan=null, $status=null, $keterangan=null){
        if(!empty($tes_id) AND !empty($grup_id) AND !empty($waktu) AND !empty($urutkan) AND !empty($status)){
			...
			if($status=='mengerjakan'){
				$query = $this->tes_user_model->get_by_tes_group_urut_tanggal($tes_id, $grup_id, $urutkan, $tanggal, $keterangan);
			}else{
				$query = $this->user_model->get_by_tes_group_urut_tanggal($tes_id, $grup_id, $urutkan, $tanggal, $keterangan);
			}
            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
                    ...
                    $worksheet->setCellValueByColumnAndRow(3, $row, $temp->email_user);
                    $worksheet->setCellValueByColumnAndRow(4, $row, stripslashes($temp->nama_user));
    ...
    function get_datatable(){
		...
		if($status=='mengerjakan'){
			$query = $this->tes_user_model->get_datatable($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal, $keterangan, $search);
			$iTotal= $this->tes_user_model->get_datatable_count($tes_id, $grup_id, $urutkan, $tanggal, $keterangan, $search)->row()->hasil;
		}else{
			$query = $this->user_model->get_datatable_hasiltes($start, $rows, $tes_id, $grup_id, $urutkan, $tanggal, $keterangan, $search);
			$iTotal= $this->user_model->get_datatable_hasiltes_count($tes_id, $grup_id, $urutkan, $tanggal, $keterangan, $search)->row()->hasil;
		}
	    ...
	    foreach ($query as $temp) {			
			...
			if(empty($temp->tesuser_id)){
				$record[] = '<b>'.stripslashes($temp->nama_user).'</b>';
			}else{
				$record[] = '<a href="#" title="Klik untuk mengetahui Detail Tes" onclick="detail_tes(\''.$temp->tesuser_id.'\')"><b>'.stripslashes($temp->nama_user).'</b></a>';
			}
			if(empty($temp->nilai)){
				$record[] = '0';
			}else{
