...
class Tes_evaluasi extends Member_Controller {
    ...
    function simpan_nilai(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($nilai>=$nilai_min AND $nilai<=$nilai_max){
                ...
                $data['tessoal_comment'] = 'Sudah di koreksi '.$this->access->get_email();

                $this->tes_soal_model->update('tessoal_id', $tessoal_id, $data);
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->modul_model->get_by_kolom('modul_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
    ...
    function get_datatable(){
		...
		$query = $this->tes_user_model->get_datatable_evaluasi($start, $rows, $tes_id, $urutkan);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->tes_user_model->get_datatable_evaluasi_count($tes_id, $urutkan)->row()->hasil;
