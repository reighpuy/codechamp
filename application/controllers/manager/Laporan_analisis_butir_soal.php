...
class Laporan_analisis_butir_soal extends Member_Controller
{
	...
	public function index()
	{
		...
		if ($query_tes->num_rows() > 0) {
			...
			$select = '<option value="kosong">Belum Ada Materi yang Dilakukan</option>';
		}
	...
	public function export()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$query_user = $this->tes_user_model->get_by_tes_group($tes, $grup);
			...
			if ($query_user->num_rows() > 0) {
				...
				foreach ($query_user as $user) {
					...
					$worksheet->setCellValueByColumnAndRow(1, $row, $user->email_user);
					$worksheet->setCellValueByColumnAndRow(2, $row, stripslashes($user->nama_user));

					//Mengambil data jawaban yang telah dikerjakan
					$query_soal_tes = $this->tes_soal_model->get_by_testuser_order_soal($user->tesuser_id);
