...
class Peserta_group extends Member_Controller {
    ...
    function tambah(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($this->user_grup_model->count_by_kolom('grup_nama', $data['grup_nama'])->row()->hasil>0){
                $status['status'] = 0;
                $status['pesan'] = 'Nama Group sudah terpakai !';
            }else{
				$this->user_grup_model->save($data);
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->user_grup_model->get_by_kolom('grup_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
    ...
    function edit(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($pilihan=='hapus'){//hapus
            	if($this->tesgrup_model->count_by_kolom('tstgrp_grup_id', $id)->row()->hasil>0){
            		$status['status'] = 0;
					$status['pesan'] = 'Group tidak dapat dihapus, Group masih digunakan Tes !';
            	}else{
            		$this->user_grup_model->delete('grup_id', $id);
					$status['status'] = 1;
                ...
                if($group_asli!=$data['grup_nama']){
                	if($this->user_grup_model->count_by_kolom('grup_nama', $data['grup_nama'])->row()->hasil>0){
						...
						$this->user_grup_model->update('grup_id', $id, $data);
    ...
    function get_datatable(){
		...
		$query = $this->user_grup_model->get_datatable($start, $rows, 'grup_nama', $search);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->user_grup_model->get_datatable_count('grup_nama', $search)->row()->hasil;
