...
class Tool_exportimport_soal extends Member_Controller {
    ...
    public function exportsoal(){
		...
		$query_modul = $this->modul_model->get_all();
		$query_topik = $this->topik_model->get_all();
		$query_soal = $this->soal_model->get_all();
		$query_jawaban = $this->jawaban_model->get_all();

		if($query_modul->num_rows()>0){
			$query_modul = $query_modul->result();
			foreach ($query_modul as $modul) {
				$sql = $sql.'INSERT INTO `modul` (`modul_id`, `modul_nama`, `modul_aktif`) VALUES (\''.$modul->modul_id.'\', \''.$modul->modul_nama.'\', '.$modul->modul_aktif.')'.$delimiter;
			}			
		}
		...
		if($query_topik->num_rows()>0){
			...
			foreach ($query_topik as $topik) {
				$sql = $sql.'INSERT INTO `topik` (`topik_id`, `topik_modul_id`, `topik_nama`, `topik_detail`, `topik_aktif`) VALUES (\''.$topik->topik_id.'\', \''.$topik->topik_modul_id.'\', \''.addslashes($topik->topik_nama).'\', \''.addslashes($topik->topik_detail).'\', '.$topik->topik_aktif.')'.$delimiter;
			}			
		}
		...
		if($query_jawaban->num_rows()>0){
			...
			foreach ($query_jawaban as $jawaban) {
				$sql = $sql.'INSERT INTO `jawaban` (`jawaban_id`, `jawaban_soal_id`, `jawaban_detail`, `jawaban_benar`, `jawaban_aktif`) VALUES (\''.$jawaban->jawaban_id.'\', \''.$jawaban->jawaban_soal_id.'\', \''.addslashes($jawaban->jawaban_detail).'\', '.$jawaban->jawaban_benar.', '.$jawaban->jawaban_aktif.')'.$delimiter;
			}			
		}

		$this->zip->add_data('codechamp-soal.sql', $sql);

		$path = $this->config->item('upload_path');
		$this->zip->read_dir($path);

		$this->zip->download('codechamp-soal.zip');
    }
    ...
    public function exportsoal(){
		...
		$prefs = array(
	        'tables'        => array('modul', 'topik', 'soal', 'jawaban'),
	        'ignore'        => array(),
	        'format'        => 'txt',
	        'filename'      => 'codechamp-soal.sql',
	        'add_drop'      => TRUE,
	        'add_insert'    => TRUE,
	        'newline'       => "\n"
		);		
		...
		$this->zip->add_data('codechamp-soal.sql', $backup);
    ...
    public function importsoal(){
        ...
        if(!empty($_FILES['userfile']['name'])){    
	        ...
	        if($this->topik_model->count_all()->row()->hasil>0){
	        	$data['error'] = 'Data Soal Gagal di Import. Masih terdapat materi aktif. Silahkan hapus Materi terlebih dahulu.';
	        }else{
	            ...
	            if (!$this->upload->do_upload()){
			        ...
			        if ($zip->open($config['upload_path'].$upload_data['file_name']) === TRUE)			        {
			            ...
			            $this->modul_model->empty_table();
			            
			            // Import SQL
			            if (file_exists($config['upload_path'].'import/codechamp-soal.sql')) {
			            	$isi_file = file_get_contents($config['upload_path'].'import/codechamp-soal.sql'); 
