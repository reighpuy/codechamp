...
class Tes_token extends Member_Controller {
    ...
    public function index($page=null, $id=null){
        ...
        $this->tes_token_model->delete_by_user_date($user_id);
		
		$query_tes = $this->tes_model->get_by_now();
    ...
    function get_datatable(){
		...
		$query = $this->tes_token_model->get_datatable($start, $rows, 'token_isi', $search, $email);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->tes_token_model->get_datatable_count('token_isi', $search, $email)->row()->hasil;
	    ...
	    foreach ($query as $temp) {			
			...
			if($temp->token_tes_id==0){
				...
				$query_tes = $this->tes_model->get_by_kolom_limit('tes_id', $temp->token_tes_id, 1);
				if($query_tes->num_rows()>0){
