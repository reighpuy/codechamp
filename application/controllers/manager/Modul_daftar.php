...
class Modul_daftar extends Member_Controller
{
			...
			if ($query_modul->num_rows() > 0) {
				...
				foreach ($query_modul as $temp) {
					...
					if ($query_topik->num_rows()) {
						$select = $select . '<optgroup label="Daftar Kategori">';
						// $select = $select.'<optgroup label="Kategori '.$temp->modul_nama.'">';
	...
	{
		if (!empty($topik_id)) {
			$query_soal = $this->soal_model->get_by_kolom('soal_topik_id', $topik_id);
			if ($query_soal->num_rows() > 0) {
				$query_soal = $query_soal->result();

				$query_topik = $this->topik_model->get_by_kolom_limit('topik_id', $topik_id, 1)->row();
				...
				foreach ($query_soal as $temp) {
					...
					$query_jawaban = $this->jawaban_model->get_by_soal($temp->soal_id);
	...
	function get_topik_by_modul($modul_id = null)
	{
		$data['data'] = 0;
		if (!empty($modul_id)) {
			$query = $this->topik_model->get_by_kolom('topik_modul_id', $modul_id);
			if ($query->num_rows() > 0) {
				$query = $query->result();
				$data['data'] = 1;
