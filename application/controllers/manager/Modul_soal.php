...
class Modul_soal extends Member_Controller {
        ...
        if($query_user->num_rows()>0){
			...
			if($query_modul->num_rows()>0){
				...
				foreach ($query_modul as $temp) {
					$query_topik = $this->topik_model->get_by_kolom_join_modul('topik_modul_id', $temp->modul_id);
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	if($id_topik=='kosong'){
	        	...
	        	if(!empty($id_soal)){
	        		 ...
	        		 * Jika soal update
	        		 */
	        		$this->soal_model->update('soal_id', $id_soal, $data);
	        	}else{
	        		/**
	        		 * Jika soal baru
	        		 */
	        	...
	        	if($upload==0){
					...
					$status['status'] = 1;
					$status['pesan'] = 'Soal berhasil disimpan<br />Pesan : '.$status['pesan_upload'];
	        	}
        	}
        }else{
            $status['status'] = 0;
    ...
    function hapus_soal(){
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	$query_soal = $this->soal_model->get_by_kolom_limit('soal_id', $id, 1);
        	if($query_soal->num_rows()>0){
        		$query_soal = $query_soal->row();

        		if($this->tes_topik_set_model->count_by_kolom('tset_topik_id', $query_soal->soal_topik_id)->row()->hasil>0){
	        		$status['status'] = 0;
	            	$status['pesan'] = 'Soal tidak bisa dihapus, Topik soal masih dipakai pada Tes.';
	        	}else{
	        		$this->soal_model->delete('soal_id', $id);
		        	$status['status'] = 1;
    ...
    function get_datatable(){
		...
		$query = $this->soal_model->get_datatable($start, $rows, 'soal_detail', $search, $topik);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->soal_model->get_datatable_count('soal_detail', $search, $topik)->row()->hasil;
	    ...
	    foreach ($query as $temp) {			
            ...
            $query_jawaban = $this->jawaban_model->count_by_kolom('jawaban_soal_id', $temp->soal_id)->row();
