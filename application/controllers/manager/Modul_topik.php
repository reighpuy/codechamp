...
class Modul_topik extends Member_Controller
{
	...
	function tambah()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			//if($this->topik_model->count_by_kolom('topik_nama', $data['topik_nama'])->row()->hasil>0){
			if ($this->topik_model->count_by_topik_modul($data['topik_nama'], $data['topik_modul_id'])->row()->hasil > 0) {
				$status['status'] = 0;
				$status['pesan'] = 'Nama Topik sudah terpakai !';
			} else {
				$this->topik_model->save($data);
	...
	function get_by_id($id = null)
	{
		$data['data'] = 0;
		if (!empty($id)) {
			$query = $this->topik_model->get_by_kolom('topik_id', $id);
			if ($query->num_rows() > 0) {
				$query = $query->row();
				$data['data'] = 1;
	...
	function hapus_daftar_topik()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			foreach ($topik_id as $kunci => $isi) {
				if ($isi == "on") {
					if ($this->tes_topik_set_model->count_by_kolom('tset_topik_id', $kunci)->row()->hasil > 0) {
						$error_hapus++;
					} else {
						...
						$this->topik_model->delete('topik_id', $kunci);
	...
	function edit()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($pilihan == 'hapus') { //hapus
				if ($this->tes_topik_set_model->count_by_kolom('tset_topik_id', $id)->row()->hasil > 0) {
					$status['status'] = 0;
					$status['pesan'] = 'Topik masih dipakai pada Tes, tidak bisa dihapus.';
				} else {
					// hapus topik di database
					$this->topik_model->delete('topik_id', $id);
				...
				if ($topik_asli != $data['topik_nama']) {
					//if($this->topik_model->count_by_kolom('topik_nama', $data['topik_nama'])->row()->hasil>0){
					if ($this->topik_model->count_by_topik_modul($data['topik_nama'], $modul_id)->row()->hasil > 0) {
						$status['status'] = 0;
						$status['pesan'] = 'Nama Topik sudah terpakai !';
					} else {
						$this->topik_model->update('topik_id', $id, $data);
	...
	function get_datatable()
	{
		...
		$query = $this->topik_model->get_datatable($start, $rows, 'topik_nama', $search, $modul);
		$iFilteredTotal = $query->num_rows();

		$iTotal = $this->topik_model->get_datatable_count('topik_nama', $search, $modul)->row()->hasil;
		...
		foreach ($query as $temp) {
			...
			$jml_soal = $this->soal_model->count_by_kolom('soal_topik_id', $temp->topik_id)->row()->hasil;
