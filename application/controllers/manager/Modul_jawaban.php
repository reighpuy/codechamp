...
class Modul_jawaban extends Member_Controller {
    ...
    public function index($id_soal=null, $id_jawaban=null){
        ...
        if($id_soal!=null){
	        ...
	        if($id_jawaban!=null){
	        	$query_jawaban = $this->jawaban_model->get_by_kolom_limit('jawaban_id', $id_jawaban, 1);
        	...
        	$query_soal = $this->soal_model->get_by_kolom_limit('soal_id', $id_soal, 1);
        	if($query_soal->num_rows()>0){
        		...
        		if($query_soal->soal_tipe!=2){
					...
					$query_topik = $this->topik_model->get_by_kolom_limit('topik_id', $query_soal->soal_topik_id, 1)->row();
    ...
    function tambah(){
        ...
        if($this->form_validation->run() == TRUE){
	        ...
	        $id_topik = $this->soal_model->get_by_kolom_limit('soal_id', $id_soal, 1)->row()->soal_topik_id;
	        ...
	        if(!empty($id_jawaban)){
	        	 ...
	        	 * Jika jawban salah
	        	 */
	        	$this->jawaban_model->update('jawaban_id', $id_jawaban, $data);
	        	$status['status'] = 1;
    ...
    function hapus_jawaban(){
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	if($this->tes_soal_jawaban_model->count_by_kolom('soaljawaban_jawaban_id', $id)->row()->hasil>0){
        		$status['status'] = 0;
            	$status['pesan'] = 'Jawaban tidak bisa dihapus, Jawaban masih dipakai dalam Tes.';
        	}else{
        		$this->jawaban_model->delete('jawaban_id', $id);
	        	$status['status'] = 1;
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->jawaban_model->get_by_kolom_limit('jawaban_id', $id, 1);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
    ...
    function get_soal_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->soal_model->get_by_kolom('soal_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
    ...
    function get_datatable(){
		...
		$query = $this->jawaban_model->get_datatable($start, $rows, 'jawaban_detail', $search, $soal);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->jawaban_model->get_datatable_count('jawaban_detail', $search, $soal)->row()->hasil;
