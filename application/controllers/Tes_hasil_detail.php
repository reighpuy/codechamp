...
class Tes_hasil_detail extends Tes_Controller {
    ...
    public function index($tesuser_id=null){
        ...
        if(!empty($tesuser_id)){
        	$query_testuser = $this->tes_user_model->get_by_kolom_limit('tesuser_id', $tesuser_id, 1);
        	if($query_testuser->num_rows()>0){
        		$query_testuser = $query_testuser->row();

        		$query_test = $this->tes_model->get_by_kolom_limit('tes_id', $query_testuser->tesuser_tes_id, 1)->row();
        		$query_user = $this->user_model->get_by_kolom_limit('user_id', $query_testuser->tesuser_user_id, 1)->row();

        		$data['tes_user_id'] = $tesuser_id;
        		$data['tes_nama'] = $query_test->tes_nama;
        		$data['tes_mulai'] = $query_testuser->tesuser_creation_time;
        		$data['user_nama'] = $query_user->nama_user;

        		$nilai = $this->tes_soal_model->get_nilai($tesuser_id)->row();
        		$data['nilai'] = $nilai->hasil.'  /  '.$query_test->tes_max_score.'  (skor / skor maksimal) ';
    ...
    function get_datatable(){
		...
		$query = $this->tes_soal_model->get_datatable($start, $rows, 'soal_detail', $search, $tesuser_id);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->tes_soal_model->get_datatable_count('soal_detail', $search, $tesuser_id)->row()->hasil;
	    ...
	    foreach ($query as $temp) {
            ...
            if($temp->soal_tipe==1){
            	$query_jawaban = $this->tes_soal_jawaban_model->get_by_tessoal($temp->tessoal_id);
	            if($query_jawaban->num_rows()>0){
	            	$query_jawaban = $query_jawaban->result();
	            	$a = 0;
	            	$jawaban_table = $jawaban_table.'
	            			<tr>
		                      	<td width="15%">Jawaban Benar</td>
		                      	<td width="10%">Pilihan Anda</td>
	            	...
	            	foreach ($query_jawaban as $jawaban) {
	            		...
	            		$jawaban_table = $jawaban_table.'
	            			<tr>
		                      	<td width="15%">'.$temp_benar.'</td>
		                      	<td width="10%">'.$temp_pilihan.'</td>
