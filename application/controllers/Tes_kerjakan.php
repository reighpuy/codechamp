...
class Tes_kerjakan extends Tes_Controller {
    ...
    function __construct(){
        ...
        $this->email_user = $this->access_tes->get_email();
        $this->user_id = $this->user_model->get_by_kolom_limit('email_user', $this->email_user, 1)->row()->user_id;
	}
    ...
    public function index($tes_id=null){
        if(!empty($tes_id)){
            ...
            $query_tes = $this->tes_user_model->get_by_user_tes_limit($this->user_id, $tes_id, 1);
            if($query_tes->num_rows()>0){
                ...
                $tessoal = $this->tes_soal_model->get_by_testuser_limit($query_tes->tesuser_id, 1)->row();
    ...
    function hentikan_tes(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if(!empty($centang)){
                ...
                $this->tes_user_model->update('tesuser_id', $tesuser_id, $data_tes);
                $this->db->query("UPDATE cc_user SET exp_user=exp_user+30 WHERE user_id=$user_id");
    ...
    function simpan_jawaban(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            //if($this->tes_user_model->count_by_status_waktu($tes_user_id)->row()->hasil>0){
            //
            // revisi 2018-11-15
            // agar waktu mengambil dari waktu php, bukan mysql
            $waktuuser = date('Y-m-d H:i:s');
            if($this->tes_user_model->count_by_status_waktuuser($tes_user_id, $waktuuser)->row()->hasil>0){

                // Mengecek apakah soal ada
                $query_soal = $this->tes_soal_model->get_by_tessoal_limit($tes_soal_id, 1);
                if($query_soal->num_rows()>0){
                    ...
                    if($query_soal->soal_tipe==1){
                        ...
                        $query_tes = $this->tes_model->get_by_kolom_limit('tes_id', $tes_id, 1)->row();

                        // Mendapatkan data jawaban
                        $query_jawaban = $this->tes_soal_jawaban_model->get_by_tessoal_answer($tes_soal_id, $jawaban)->row();

                        // Mengupdate pilihan jawaban benar
                        $data_jawaban['soaljawaban_selected']=1;
                        $this->tes_soal_jawaban_model->update_by_tessoal_answer($tes_soal_id, $jawaban, $data_jawaban);
                        // Mengupdate pilihan jawaban salah
                        $data_jawaban['soaljawaban_selected']=0;
                        $this->tes_soal_jawaban_model->update_by_tessoal_answer_salah($tes_soal_id, $jawaban, $data_jawaban);
                        ...
                        $this->tes_soal_model->update('tessoal_id', $tes_soal_id, $data_tes_soal);
    ...
    function get_tes_info($tes_id=null){
        ...
        if(!empty($tes_id)){
            ...
            if($query_tes->num_rows()>0){
                ...
                $data['tes_dijawab'] = $this->tes_soal_model->count_by_tesuser_dijawab($query_tes->tesuser_id)->row()->hasil.' Soal';
                $data['tes_blum_dijawab'] = $this->tes_soal_model->count_by_tesuser_blum_dijawab($query_tes->tesuser_id)->row()->hasil.' Soal';
            }
        }
    ...
    /**
     * Mendapatkan data tes_soal berdasarkan tessoal_id
    ...
    function get_tes_soal_by_tessoal($tessoal_id=null){
        $data['data'] = 0;
        if(!empty($tessoal_id)){
            $query_tes_soal = $this->tes_soal_model->get_by_kolom_limit('tessoal_id', $tessoal_id, 1);
    ...
    function update_tes_soal_ragu($tessoal_id=null, $ragu=null){
        ...
        if(!empty($tessoal_id)){
            ...
            $this->tes_soal_model->update('tessoal_id', $tessoal_id, $data_tes_soal);
        }
    ...
    private function get_daftar_soal($tes_id=null){
        ...
        $data_soal = '';
        if(!empty($tes_id)){
            $query_tes = $this->tes_user_model->get_by_user_tes_limit($this->user_id, $tes_id);

            if($query_tes->num_rows()>0){
                $query_tes = $query_tes->row();

                $query_soal = $this->tes_soal_model->get_by_testuser($query_tes->tesuser_id);
    ...
    private function get_soal($tessoal_id=null, $tesuser_id=null){
        ...
        if(!empty($tessoal_id) AND !empty($tesuser_id)){
            ...
            //if($this->tes_user_model->count_by_status_waktu($tesuser_id)->row()->hasil>0){
            //
            // revisi 2018-11-15
            // agar waktu mengambil dari waktu php, bukan mysql
            $waktuuser = date('Y-m-d H:i:s');
            if($this->tes_user_model->count_by_status_waktuuser($tesuser_id, $waktuuser)->row()->hasil>0){
                $data['data'] = 1;
                $query_soal = $this->tes_soal_model->get_by_tessoal_limit($tessoal_id, 1);
                $soal = '';
                if($query_soal->num_rows()>0){
                    ...
                    if($query_soal->soal_tipe==1){
                        $query_jawaban = $this->tes_soal_jawaban_model->get_by_tessoal($query_soal->tessoal_id);
